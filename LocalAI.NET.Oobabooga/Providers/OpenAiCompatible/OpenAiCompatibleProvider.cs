...
{
    public class OpenAiCompatibleProvider : IOpenAiCompatibleProvider
    {
        ...
        public async Task<string> CompleteAsync(
            string prompt,
            OobaboogaCompletionOptions? options = null,
            CancellationToken cancellationToken = default)
        {
            ...
            var result = JsonConvert.DeserializeObject<OobaboogaCompletionResponse>(responseContent, _jsonSettings);
            return result?.Choices.FirstOrDefault()?.Text ?? string.Empty;
        }

        public async Task<OobaboogaChatCompletionResponse> ChatCompleteAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            OobaboogaChatCompletionRequest request = new OobaboogaChatCompletionRequest
            {
                Model = options?.ModelName ?? _modelName,
                Messages = messages,
            ...
            OobaboogaChatCompletionResponse? result = JsonConvert.DeserializeObject<OobaboogaChatCompletionResponse>(responseContent, _jsonSettings);
            return result ?? new OobaboogaChatCompletionResponse();
        }
        ...
        public async IAsyncEnumerable<OobaboogaChatMessage> StreamChatCompletionAsync(
            List<OobaboogaChatMessage> messages,
            OobaboogaChatCompletionOptions? options = null,
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                {
                    yield return new OobaboogaChatMessage
                    {
        ...
        private async Task<HttpResponseMessage> GetStreamResponseAsync(
            List<OobaboogaChatMessage> messages,
            OobaboogaChatCompletionOptions? options,
        ...
        private HttpRequestMessage CreateStreamRequest(List<OobaboogaChatMessage> messages, OobaboogaChatCompletionOptions? options)
        {
            var request = new OobaboogaChatCompletionRequest
            {
                Model = options?.ModelName ?? _modelName,
                Messages = messages,
                Temperature = options?.Temperature,
